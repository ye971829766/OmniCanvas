...
// Set the artboard dimensions and background. Call first when starting a new design so layout coordinates make sense. Common sizes: social 1080x1080, story 1080x1920, poster 1240x1754, banner 1920x1080.
type set_frame = (_: {
  width: number,
  height: number,
  // CSS color, e.g. "#ffffff" or "#111"
  background?: string,
}) => any;

// Add a new artboard/frame container to the canvas to start a new composition or layout. Returns a refId to be used as parentId for child elements.
type add_frame = (_: {
  width: number,
  height: number,
  // CSS color, e.g. "#ffffff"
  background?: string,
  x?: number,
  y?: number,
}) => any;

// Add a text layer (title, caption, body copy) to the canvas.
type add_text = (_: {
  text: string,
  x?: number,
  y?: number,
  ...
  // Plain number only, e.g. 1.2 or 44. Do not pass an object.
  lineHeight?: number,
  // Plain number only. Do not pass an object.
  letterSpacing?: number,
  // Plain number from 0 to 1.
  opacity?: number,
  // Parent node refId, e.g. "agent_frame" to place the text inside the artboard.
  parentId?: string,
}) => any;

// Add a rectangle (background block, card, color panel, divider).
type add_rect = (_: {
  x: number,
  y: number,
  width: number,
  ...
  // 0-1, default 1
  opacity?: number,
  // Parent node refId, e.g. "agent_frame" to place the rect inside the artboard.
  parentId?: string,
  gradient?: {
    from: string,
    to: string,
...
// Update an existing canvas node by refId: move, resize, restyle text, etc. Use the refId returned by a previous tool.
type update_node = (_: {
  refId: string,
  x?: number,
  y?: number,
...
// Remove a canvas node by refId.
type remove_node = (_: {
  refId: string,
}) => any;

// Get a summary of all nodes currently on the canvas, including those added by the user manually. Use this to check what exists before modifying, repositioning, or removing nodes. Returns a list of nodes with their tag, position, size, and key properties.
type query_canvas = (_: {} & {}) => any;

// Focus the canvas viewport on a specific node by its refId (zooms in and centers on it). Use this to inspect elements clearly or direct user attention to a node.
type focus_node = (_: {
  // The refId of the element to focus on.
  refId: string,
}) => any;

// Export a specific element or container by its refId as a PNG image. Returns the image as a base64 Data URI, allowing you to "see" what it looks like.
type export_node_image = (_: {
  // The refId of the element or container to capture.
  refId: string,
}) => any;

// Generate an image from a text prompt and place it on the canvas. Use for illustrations, photos, backgrounds, icons, concept art. Prompt should be descriptive; English usually yields the best quality.
type generate_image = (_: {
  // Detailed description of the image
  prompt: string,
  // Optional image model id. Usually omit unless the user selected one.
  model?: string,
  aspectRatio?: '1:1' | '16:9' | '9:16' | '4:3' | '3:4',
  // e.g. "1024x1024" (optional)
  size?: string,
  // Optional provider quality such as auto, low, medium, high, standard, hd, 1K, 2K.
  quality?: string,
  // optional style hint
  style?: string,
  // Optional array of refIds of existing canvas elements or absolute URLs to use as visual reference for image-to-image/edit generation.
  refImages?: string[],
  x?: number,
  y?: number,
  width?: number,
  height?: number,
  // Parent node refId, e.g. "agent_frame" to place the image inside the artboard.
  parentId?: string,
}) => any;

// Generate a short video from a text prompt (optionally from a reference image) and place it on the canvas. Use for animated banners, motion backgrounds, short clips.
type generate_video = (_: {
  prompt: string,
  // Optional video model id. Usually omit unless the user selected one.
  model?: string,
  aspectRatio?: '16:9' | '9:16' | '1:1',
  size?: string,
  // clip length, e.g. "5"
  seconds?: string,
  // Optional array containing the refId of the reference image on the canvas to use as input reference for video generation.
  refImages?: string[],
  x?: number,
  y?: number,
  // Parent node refId, e.g. "agent_frame" to place the video inside the artboard.
  parentId?: string,
}) => any;

// Automatically rearrange all elements on the canvas using a layout strategy (hero, centered, split-horizontal, split-vertical, grid, hero-overlay). This recalculates x, y, width, and height for all elements for a premium layout.
type auto_layout = (_: {
  // The visual structure to apply.
  layoutHint: 'hero' | 'centered' | 'split-horizontal' | 'split-vertical' | 'grid' | 'hero-overlay',
}) => any;

// Align multiple selected nodes (left, center, right, top, middle, bottom) relative to their bounding box.
type align_nodes = (_: {
  // The refIds of the nodes to align.
  refIds: string[],
  alignment: 'left' | 'center' | 'right' | 'top' | 'middle' | 'bottom',
}) => any;

// Distribute multiple selected nodes evenly in horizontal or vertical spacing.
type distribute_nodes = (_: {
  // The refIds of the nodes to distribute.
  refIds: string[],
  direction: 'horizontal' | 'vertical',
}) => any;

// Define the brand design system for this session. The planner and style tools will prioritize these choices (primary/secondary/accent/text/background colors, fonts, and keywords).
type set_brand = (_: {
  ...
  fontFamily?: string,
  // Style mood keywords, e.g. ["vintage", "tech"]
  styleKeywords?: string[],
}) => any;

// Instantly update the color theme of all elements on the canvas to match a specific palette ID (warm-cafe, tech-blue, nature-green, sunset-warm, luxury-dark, pastel-soft, minimal-mono, vibrant-pop, ocean-calm, earth-tone).
type apply_palette = (_: {
  // The ID of the color palette to apply.
  paletteId: 'warm-cafe' | 'tech-blue' | 'nature-green' | 'sunset-warm' | 'luxury-dark' | 'pastel-soft' | 'minimal-mono' | 'vibrant-pop' | 'ocean-calm' | 'earth-tone',
}) => any;

// Review the current canvas design and automatically adjust elements that look misplaced or poorly sized. Checks: title readability, image sizing (should not fill 100%), spacing between elements, CTA button prominence. Call this after creating complex designs to ensure quality.
type review_and_adjust = (_: {} & {}) => any;
