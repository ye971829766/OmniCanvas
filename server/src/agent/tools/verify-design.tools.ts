...
export const verifyDesignTool: AgentTool = {
  ...
  async execute(input: { refId: string; requirements: string }, ctx: ToolContext): Promise<ToolResult> {
    ctx.sink.emit({ type: 'progress', tool: 'verify_design', message: `正在视觉质检：${input.refId}` });

    const result = await runMCotLoop(ctx, input.refId, input.requirements);
