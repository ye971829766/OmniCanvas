...
后面计划会增加精细化的属性编辑器，如编辑圆角、形状、路径等，暂时与 [自动布局](../flow/index.md) 元素不能混用。
:::

## 安装插件

需要安装 编辑器插件、[resize 插件](../resize/index.md) 才能使用，[点此访问 Github 仓库](https://github.com/leaferjs/leafer-in/tree/main/packages/editor)。
...
```html [editor.min]
<script src="https://unpkg.com/@leafer-in/editor@2.1.4/dist/editor.min.js"></script>
...
```html [editor]
<script src="https://unpkg.com/@leafer-in/editor@2.1.4/dist/editor.js"></script>
...
设置需要编辑的元素， 默认通过 [编辑选择器](./config/select.md#selector-boolean) 自动选取 [editable](../../../reference/UI/editable.md) 元素（不用设置 draggable ）。
...
[编辑器配置](./config/base.md)，应用初始化时通过 `editor: {}` 配置对象传入。

可通过 [app.editor.config](./config/base.md) 实时修改 ( 特殊情况调用 [updateEditTool()](./Editor/update.md) 方法立即更新 )。

## 次要属性

### hoverTarget: [`UI`](../../../reference/display/UI.md)

设置 hover 状态的元素， 默认通过 [编辑选择器](./config/select.md#selector-boolean) 自动选取 [editable](../../../reference/UI/editable.md) 元素。
...
指定编辑器 [dimOthers](./config/style.md#bright-boolean) 配置的容器节点，用于确定淡化内容的范围，支持数组，默认为 `targetLeafer`。
...
| [skewOf()](./Editor/transform.md)             | 围绕 [element](#element-ui) 元素的原点 origin（ [box 坐标](../../../guide/advanced/coordinate.md#box-坐标系) ）倾斜选中元素 <badge>增量操作</badge> |

## 配置 editor

### [基础](./config/base.md) &nbsp; &nbsp; [事件](./config/event.md) &nbsp; &nbsp; [样式](./config/style.md) &nbsp; &nbsp; [按钮组](./config/buttons.md) &nbsp; &nbsp; [光标](./config/cursor.md) &nbsp; &nbsp; [选择](./config/select.md) &nbsp; &nbsp; [控制](./config/control.md) &nbsp; &nbsp; [启用](./config/enable.md) &nbsp; &nbsp; [内部编辑器](./config/innerEditor.md)

### resize 元素

编辑器默认通过修改元素的宽高、路径坐标来调整大小，与专业级的设计工具效果一致。

同时也支持配置通过 fontSize、缩放属性来调整大小， 了解 [editSize](./config/base.md)。

## 辅助

### [快捷键](./Editor/shortcut.md)

### [历史记录](./Editor/history.md)
...
| [EditorRotateEvent](./event/EditorRotateEvent.md) | 旋转事件           |
| [EditorSkewEvent](./event/EditorSkewEvent.md)     | 倾斜事件           |
